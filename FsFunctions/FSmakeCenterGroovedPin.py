...
def makeCenterGroovedPin(self, fa):
    ...
    fm.Reset()
    groove_d = d_1 / 12
    groove_r = groove_d / 2 + groove_l**2 / (8 * groove_d)
    fm.AddPoint(d_1 / 2 - groove_d, 0.0)
    fm.AddPoint(d_1 / 2 - groove_d + groove_r, math.tan(math.radians(35)) * groove_r)
    fm.AddPoint(d_1 / 2 - groove_d + groove_r, -math.tan(math.radians(35)) * groove_r)
    groove_cutter = fm.GetFace().revolve(
        Base.Vector(d_1 / 2 - groove_d + groove_r, 0.0, 0.0),
        Base.Vector(0.0, 0.0, 1.0),
        360,
    )
    groove_cutter.rotate(Base.Vector(0.0, 0.0, 0.0), Base.Vector(1.0, 0.0, 0.0), 90)
    groove_cutter.translate(
        Base.Vector(-2 * (d_1 / 2 - groove_d + groove_r), 0.0, -length / 2)
    )
    for i in range(3):
        shape = shape.cut(
