...
To attach a fastener to a feature created with part design, it must be attached to the body, rather then one of its inner elements. To do so, first switch the "Display Mode" of the body from "Through" to "Tip". This can be found in the "View" tab of the Body's properties panel. You might also need to change "selectable" to "True" in the same place. To continue editing the Body, switch back to "Through" 
