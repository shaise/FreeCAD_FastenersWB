...
class Screw(object):
  ...
  def createScrew(self, ST_text, ND_text, NL_text, threadType, shapeOnly = False):
    ...
    #self.symThread = self.SymbolThread.isChecked()
    #self.rThread = self.RealThread.isChecked()
    FreeCAD.Console.PrintMessage("Calling Cython module\n")
    if threadType == 'real':
      self.rThread = True
    else:
      self.rThread = False
  ...
  def moveScrew(self, ScrewObj_m):
   ...
   #neurotensin    for o in Gui.Selection.getSelectionEx():
       for o in FreeCAD.Gui.Selection.getSelectionEx():
  ...
  def makeInnerThread_2(self, d, P, rotations, da, l):
    ...
    if da <> None:
      ...
      bot_face.reverse()
 #neurotensin
  ##wrong    cham_Shell = Part.Shell(cham_faces)
      for flaeche in BotShell.Faces:
        TheFaces.append(flaeche)
      if da <> None:
##neurotensin this is impossible to reach?
       ## for flaeche in cham_Shell.Faces:
        ##  TheFaces.append(flaeche)
      #else:  
        TheFaces.append(bot_face)
